...
<script>
  ...
  export default {
    ...
    mixins: [liteMixin, paymentMixin, entitiesMixin, appointmentMixin, imageMixin, dateMixin, durationMixin, notifyMixin, customerMixin, backendEventMixin, commonEventMixin],
    ...
    methods: {
      ...
      getParsedEmployees () {
        ...
        let minAvailableDateTime = moment(this.$root.settings.slotDateConstraints.minDate, 'YYYY-MM-DD HH:mm:ss')
        ...
        employees.forEach(function (employee) {
          ...
          let minDateDayIndex = (paramsDates.indexOf(minAvailableDate.format('YYYY-MM-DD')) !== -1) ? moment(minAvailableDate).day() : null

          if (minDateDayIndex !== null && that.$root.settings.role === 'customer') {
            employee.weekDayList.forEach(function (day, index) {
              if ((day.dayIndex === 7 ? 0 : day.dayIndex) === minDateDayIndex) {
                day.startTime = minAvailableDateTime.format('HH:mm:ss')
              }
            })
          }
        })
      ...
      getEvents () {
        ...
        this.$http.get(`${this.$root.getAjaxUrl}/events`, {
          params: Object.assign(JSON.parse(JSON.stringify(this.params)), {providers: providers, status: 'approved'})
        })
          .then(response => {
            let that = this
            ...
            response.data.data.events.forEach(function (event) {
              ...
              event.providers.forEach(function (employee) {
                employeesInfo += '<div ' + employeesStyle + '><img src="' + that.pictureLoad(employee, true) + '">' + employee.firstName + ' ' + employee.lastName + '</div>'
              })
                ...
                event.providers.forEach(function (provider, index) {
                  if (that.$refs.calendar.fireMethod('getView').name === 'timelineDay' || that.$refs.calendar.fireMethod('getView').name === 'day' || index === 0) {
                    that.events.push({
                      ...
                      location: event.locationId ? that.getLocationById(event.locationId).address : event.customLocation,
                      status: event.status,
                      eventOverlap: false,
                ...
                if (['rejected', 'canceled'].indexOf(appointmentDays[appointmentDay].appointments[i].status) === -1) {
                  ...
                  if (that.$root.settings.role !== 'customer' || (that.$root.settings.role === 'customer' && ['rejected', 'canceled'].indexOf(appointmentDays[appointmentDay].appointments[i].bookings[0].status) === -1)) {
                    ...
                    that.events.push({
                      ...
                      employeesInfo: '<div style="display: inline-block;"><div><img src="' + that.pictureLoad(employee, true) + '">' + employee.firstName + ' ' + employee.lastName + '</div></div>',
                      location: location ? location.address : '',
                      status: appointment.status,
                      eventOverlap: false,
                      resourceId: employee.id.toString(),
                      editable: that.canWriteAppointments() && (that.$root.settings.role === 'customer' ? service.minCapacity === 1 && service.maxCapacity === 1 && !appointment.past : true),
      ...
      eventDrop (event, delta, revertFunc) {
        ...
        if (!this.canWriteAppointments() || droppedInPast || !droppedInWorkingHours || droppedInBreak || eventInDroppedTime.length !== 0 || droppedOutSchedule || droppedInAppointment) {
          ...
          this.$confirm(
            ...
            .then(() => {
              this.appointmentsFetchedFiltered = false
              this.$http.post(`${this.$root.getAjaxUrl}/appointments/time/${draggedEvent.id}`, {
      ...
      setGlobalBusinessHours (employees) {
        ...
        this.events.forEach(function (event) {
          let eventStart = moment.duration(moment(event.start).format('HH:mm:ss'))

          if (eventStart < minTime) {
            minTime = eventStart
          }

          if (eventStart > maxTime) {
            maxTime = eventStart
          }
        })
