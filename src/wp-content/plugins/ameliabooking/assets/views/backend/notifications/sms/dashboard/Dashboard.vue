...
<script>
  ...
  export default {
    ...
    mounted () {
      ...
      if ('payPalCheckoutSuccess' in urlParams) {
    ...
    methods: {
      ...
      completePayment () {
        ...
        if (urlParams.payPalCheckoutSuccess === 'true') {
          this.fetched = false
          this.paymentComplete.payerId = urlParams.PayerID
          this.paymentComplete.paymentId = urlParams.paymentId
          this.paymentComplete.amount = urlParams.amount
          this.sendAmeliaSmsApiRequest('paymentComplete', this.onPaymentCompleteSuccess, this.onPaymentSuccessError)
        } else {
          this.fetched = true
          this.getUserInfo()
          this.notify(this.$root.labels.error, this.$root.labels.cant_recharge_balance, 'error')
        }
      },

      onPaymentCompleteSuccess () {
        let redirectURL = this.removeURLParameter(window.location.href, 'payPalCheckoutSuccess')
        redirectURL = this.removeURLParameter(redirectURL, 'notificationTab')
        redirectURL = this.removeURLParameter(redirectURL, 'amount')
        redirectURL = this.removeURLParameter(redirectURL, 'paymentId')
        redirectURL = this.removeURLParameter(redirectURL, 'token')
        redirectURL = this.removeURLParameter(redirectURL, 'PayerID')
        history.pushState(null, null, redirectURL + '#/notifications')
        this.getUserInfo()
        this.notify(this.$root.labels.success, this.$root.labels.balance_recharged, 'success')
      },

      onPaymentSuccessError () {
