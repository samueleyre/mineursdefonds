            ...
            >
              <el-form class="" :action="exportPaymentsAction" method="POST">
                <el-row :gutter="16">
                  ...
                  <!-- Date Filter -->
                  <el-col :md="24" :lg="4" class="v-calendar-column">
            ...
            >
              ...
              <el-collapse>
                <el-collapse-item
                    ...
                    v-if="payment.providers.length === 0 || (payment.providers.length && (appointmentProvider = getProviderById(payment.providers[0].id)) && appointmentProvider !== null)"
                    :key="payment.id"
                    :name="payment.id"
                    class="am-finance"
                >
                        ...
                        <el-col :lg="10">
                          <el-row :gutter="10" class="am-finance-flex-row-middle-align">
                            ...
                            <el-col :lg="6" :sm="4" class="align-right">
                              <div @click.stop>
                                <el-button @click="showDialogEditPayment(payment.id, payment.appointmentId, payment.eventId)">
                                  {{ $root.labels.details }}
                                </el-button>
                              </div>
                            </el-col>
    ...
    methods: {
      ...
      getFinanceOptions () {
        ...
        this.$http.get(`${this.$root.getAjaxUrl}/entities`, {
          params: {
            types: ['categories', 'employees', 'customers', 'events']
          }
        })
          .then(response => {
      ...
      getAppointment (paymentId, appointmentId) {
        this.$http.get(`${this.$root.getAjaxUrl}/appointments/` + appointmentId)
          .then(response => {
            this.selectedPaymentModalData = this.getPaymentData(paymentId, response.data.data.appointment, null)
      ...
      getEvent (paymentId, eventId) {
        this.$http.get(`${this.$root.getAjaxUrl}/events/` + eventId)
          .then(response => {
            this.selectedPaymentModalData = this.getPaymentData(paymentId, null, response.data.data.event)
            this.bookingFetched = true
          })
          .catch(e => {
            console.log(e.message)
      ...
      duplicateCouponCallback: function () {},
      ...
      showDialogEditPayment (paymentId, appointmentId, eventId) {
        ...
        if (eventId) {
          this.getEvent(paymentId, eventId)
        }
      },
